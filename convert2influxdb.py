...
for row in reader:
    ...
    if len(row) > 1 and 'usage' in row[0]:
        if 'gas' in row[0]:
            ...
            print "energy,energy_type=gas,value_type=usage value=%s %i" % (row[2], timestamp)
            if row[4][1:] != "":
            ...
            print "energy,energy_type=electric,value_type=usage value=%s %i" % (row[4], timestamp)
            if row[6][1:] != "":
                print "energy,energy_type=electric,value_type=cost value=%s %i" % (row[6][1:], timestamp)
